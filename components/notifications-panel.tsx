...
export default function NotificationsPanel({ onClose, onMarkAllAsRead }: NotificationsPanelProps) {
  ...
  const fetchNotifications = async () => {
    try {
      setIsLoading(true)
      const response = await fetch('http://solicitud-permisos.onrender.com/8000/requests')
  ...
  const updateNotificationStatus = async (notificationId: number, currentStatus: number) => {
    try {
      ...
      const response = await fetch(`http://solicitud-permisos.onrender.com/requests/${notificationId}/notifications`, {
        method: 'PUT',
